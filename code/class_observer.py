...
class Observer(object):
    ...
    def evaluate_results(self, transport_network, household_list,
        ...
        if per_firm:
            ...
            extra_spending_per_firm = spending_per_firm_per_t.groupby(['time_step', 'firm_id'])[
                'spending'].sum().unstack('firm_id')
            extra_spending_per_firm = extra_spending_per_firm.sum() - extra_spending_per_firm.shape[
                0] * extra_spending_per_firm.iloc[0, :]
            ...
            consumption_loss_per_firm = consumption_per_firm_per_t.groupby(['time_step', 'firm_id'])[
                'consumption'].sum().unstack('firm_id')
            consumption_loss_per_firm = -(consumption_loss_per_firm.sum() - consumption_loss_per_firm.shape[
                0] * consumption_loss_per_firm.iloc[0, :])
