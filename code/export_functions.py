...
def exportDistrictSectorTable(filtered_district_sector_table, export_folder):
    filtered_district_sector_table.to_csv(os.path.join(export_folder, 'filtered_district_sector_table.csv'),
                                          index=False)
...
def exportCountryTable(country_list, export_folder):
    country_table = pd.DataFrame({
        ...
        'purchases_from_countries': [
            sum([value if isinstance(key, str) else 0 for key, value in country.purchase_plan.items()]) for country in
            country_list],
        'purchases_from_firms': [
            sum([value if isinstance(key, int) else 0 for key, value in country.purchase_plan.items()]) for country in
            country_list]
    })
...
def analyzeSupplyChainFlows(sc_network, firm_list, export_folder):
    # Collect all flows
    io_flows = [[sc_network[edge[0]][edge[1]]['object'].delivery, sc_network[edge[0]][edge[1]]['object'].supplier_id,
                 sc_network[edge[0]][edge[1]]['object'].buyer_id] for edge in sc_network.edges]
    io_flows = pd.DataFrame(columns=['quantity', 'supplier_id', 'buyer_id'], data=io_flows)

    # Analyze domestic B2C flows
    domestic_flows = io_flows[(io_flows['supplier_id'].apply(lambda x: isinstance(x, int))) & (
    ...
    domestic_b2b_flows_per_sector = domestic_b2b_flows.groupby(['from_sector', 'to_sector'])[
    ...
    for i in range(domestic_b2b_flows_per_sector.shape[0]):
        observed_io_matrix.loc[
            domestic_b2b_flows_per_sector['from_sector'].iloc[i], domestic_b2b_flows_per_sector['to_sector'].iloc[i]] = \
        domestic_b2b_flows_per_sector['quantity'].iloc[i]

    # Analyze import B2B flows
    import_flows = io_flows[(io_flows['supplier_id'].apply(lambda x: isinstance(x, str))) & (
        io_flows['buyer_id'].apply(lambda x: isinstance(x, int)))]
    ...
    export_flows = io_flows[(io_flows['supplier_id'].apply(lambda x: isinstance(x, int))) & (
        io_flows['buyer_id'].apply(lambda x: isinstance(x, str)))].copy()
    ...
    transit_flows = io_flows[(io_flows['supplier_id'].apply(lambda x: isinstance(x, str))) & (
        io_flows['buyer_id'].apply(lambda x: isinstance(x, str)))].copy()
    transit_countries = pd.Series(
        list(set(transit_flows['supplier_id']) | set(transit_flows['buyer_id']))).sort_values().tolist()
    ...
    for i in range(transit_flows.shape[0]):
        country_to_country_transit_matrix.loc[transit_flows['supplier_id'].iloc[i], transit_flows['buyer_id'].iloc[i]] = \
        transit_flows['quantity'].iloc[i]

    # Form final consumption
    final_consumption = domestic_b2c_flows_per_sector.append(
        pd.DataFrame(index=['import'], data={'from_sector': 'import', 'quantity': import_b2c_flows['quantity'].sum()}))
    ...
    if False:
        ...
        domestic_b2b_flows_per_region = domestic_b2b_flows.groupby(['from_region', 'to_region'])[
            'quantity'].sum().reset_index()
        ...
        for i in range(domestic_b2b_flows_per_region.shape[0]):
            region_to_region_io_matrix.loc[
                domestic_b2b_flows_per_region['from_region'].iloc[i], domestic_b2b_flows_per_region['to_region'].iloc[
                    i]] = domestic_b2b_flows_per_region['quantity'].iloc[i]
    ...
    if False:  # legacy, should be removed, we shall do these kind of analysis outside of the core model
        region_to_region_io_matrix.to_csv(os.path.join(export_folder, 'initial_region_to_region_io_matrix.csv'),
                                          index=True)
    country_to_country_transit_matrix.to_csv(os.path.join(export_folder, 'initial_transit_matrix.csv'), index=True)
    import_b2b_flows_per_country.to_csv(os.path.join(export_folder, 'initial_import_b2b_flows_per_country.csv'),
                                        index=False)
...
def exportTimeSeries(observer, export_folder):
    ...
    firm_avinventoryduration_ts = pd.DataFrame(
        {t: \
             {firm_id: sum(val['inventory_duration'].values()) / len(val['inventory_duration'].values()) for
              firm_id, val in observer.firms[t].items()}
         for t in observer.firms.keys()}
    ).transpose()
