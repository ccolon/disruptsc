...
def addHouseholdsForFirms(firm_table, household_table, 
    ...
    adminunit_data = gpd.read_file(filepath_adminunit_data)
    tot_pop = adminunit_data['population'].sum()
...
def defineHouseholds(sector_table, filepath_adminunit_data,
    ...
    cond = pd.Series(True, index=adminunit_data.index)
    if pop_density_cutoff > 0:
        cond = cond & (adminunit_data['pop_density'] >= pop_density_cutoff)
    if pop_cutoff > 0:
        cond = cond & (adminunit_data['population'] >= pop_cutoff)
    ...
    # compute final demand per adminunit
    rel_pop = household_table['population'] / adminunit_data['population'].sum()
    final_demand_each_household = final_demand_each_household.multiply(rel_pop, axis='index')
    # add to household table
    ...
    dic_selectAdminunit_to_points = household_table.set_index('admin_code')['geometry'].to_dict()
    # Select road node points
    road_nodes = transport_nodes[transport_nodes['type'] == "roads"]
    # Create dic
    dic_adminunit_to_roadNodeId = {
