...
from DisruptSC.functions.functions import congestion_function


class TransportNetwork(nx.Graph):
    def __init__(self, incoming_graph_data=None, **attr):
        super().__init__(incoming_graph_data, **attr)
        self.congestionned_edges = None
        self.mode_weights = None

    def add_transport_node(
            self, node_id, all_nodes_data
    ...
    def giveRouteCostWithCongestion(self, route):
        ...
        for segment in route:
            if len(segment) == 2:  # only edges have costs
                if self[segment[0]][segment[1]]["type"] != "virtual":
                    time_cost += self[segment[0]][segment[1]][
                                     "cost_variability"
                                 ] + self[segment[0]][segment[1]]["cost_travel_time"] * (
                                         1 + self[segment[0]][segment[1]]["congestion"]
                                 )
        return time_cost
    ...
    def defineWeights(self, route_optimization_weight, logistics_modes):
        ...
        The idea is to weight more or less different part of the network to "force" agent to choose one mode or the
        other. Since road needs always to be taken, we define a smaller burden.
        ...
        self.mode_weights = [
            logistic_mode + "_weight" for logistic_mode in list(logistics_modes.keys())
        ]
        # initialize the weights
        other_mode_burden = 1e10
        for edge in self.edges:
            self[edge[0]][edge[1]]["weight"] = self[edge[0]][edge[1]][
                route_optimization_weight
            ]
            self[edge[0]][edge[1]]["capacity_weight"] = self[edge[0]][edge[1]][
                route_optimization_weight
            ]
            for logistic_mode, logistic_links in logistics_modes.items():
                self[edge[0]][edge[1]][logistic_mode + "_weight"] = self[edge[0]][
                    edge[1]
                ][route_optimization_weight]
                cond_type = (
                        self[edge[0]][edge[1]]["type"]
                        not in logistic_links["accepted_modes"]
                )
                cond_multimodes = (self[edge[0]][edge[1]]["type"] is "multimodal") and (
                        self[edge[0]][edge[1]]["multimodes"]
                        not in logistic_links["accepted_multimodal_links"]
                )
                if cond_type or cond_multimodes:
                    self[edge[0]][edge[1]][
                        logistic_mode + "_weight"
                        ] = other_mode_burden
                # if self[edge[0]][edge[1]]['type'] == "airways": toprint = {weight_type: value for weight_type,
                # value in self[edge[0]][edge[1]].items() if "weight" in weight_type} print(self[edge[0]][edge[1]][
                # 'type'], self[edge[0]][edge[1]]['multimodes'], edge[0], edge[1], toprint)
    ...
    def locate_firms_on_nodes(self, firm_list, transport_nodes):
        ...
        for firm in firm_list:
            ...
            transport_nodes.loc[
                transport_nodes["id"] == firm.odpoint, "firms_there"
            ] += "," + str(firm.pid)
    ...
    def locate_households_on_nodes(self, household_list, transport_nodes):
        ...
        for household in household_list:
            ...
            transport_nodes.loc[
                transport_nodes["id"] == household.odpoint, "household_there"
            ] = household.pid
    ...
    def provide_shortest_route(self, origin_node, destination_node, route_weight):
        ...
        if origin_node not in self.nodes:
            ...
            route = [[(sp[0],)]] + [
                [(sp[i], sp[i + 1]), (sp[i + 1],)] for i in range(0, len(sp) - 1)
            ]
    ...
    def sum_indicator_on_route(self, route, indicator, detail_type=False):
        ...
        if detail_type:
            for edge in all_edges:
                new_edge = {"id": self[edge[0]][edge[1]]["id"], "type": self[edge[0]][edge[1]]["type"],
                            "multimodes": self[edge[0]][edge[1]]["multimodes"],
                            "special": self[edge[0]][edge[1]]["special"], indicator: self[edge[0]][edge[1]][indicator]}
                details += [new_edge]
    ...
    def get_undisrupted_network(self):
        available_nodes = [
            node for node in self.nodes if self._node[node]["disruption_duration"] == 0
        ]
        ...
        for edge in self.edges:
            if self[edge[0]][edge[1]]["type"] == "virtual":
                ...
                if self[edge[0]][edge[1]]["id"] in disruption["edge"]:
                    logging.info(
                        "Road edge "
                        + str(self[edge[0]][edge[1]]["id"])
                        + " gets disrupted for "
                        + str(disruption["duration"])
                        + " time steps"
                    )
                    self[edge[0]][edge[1]]["disruption_duration"] = disruption[
                        "duration"
                    ]
    ...
    def transport_shipment(self, commercial_link):
        ...
        for route_segment in route_to_take:
            if len(route_segment) == 2:  # pass shipments to edges
                self[route_segment[0]][route_segment[1]]["shipments"][
                    commercial_link.pid
                ] = {
    ...
    def update_load_on_route(self, route, load):
        ...
        for edge in edges_along_the_route:
            ...
            if (
                    self[edge[0]][edge[1]]["current_load"]
                    > self[edge[0]][edge[1]]["capacity"]
            ):
                logging.info(
                    "Edge "
                    + str(edge)
                    ...
                    + str(self[edge[0]][edge[1]]["capacity"])
                    + ")"
                )
    ...
    @staticmethod
    def check_edge_in_route(route, searched_edge):
        all_edges = [item for item in route if len(item) == 2]
        for edge in all_edges:
    ...
    def remove_shipment(self, commercial_link):
        ...
        for route_segment in route_to_take:
            if len(route_segment) == 2:  # segment is an edge
                if (
                        commercial_link.pid
                        in self[route_segment[0]][route_segment[1]]["shipments"].keys()
                ):
                    del self[route_segment[0]][route_segment[1]]["shipments"][
                        commercial_link.pid
                    ]
            elif len(route_segment) == 1:  # segment is a node
                if (
                        commercial_link.pid
                        in self._node[route_segment[0]]["shipments"].keys()
                ):
                    del self._node[route_segment[0]]["shipments"][commercial_link.pid]

    def compute_flow_per_segment(self, flow_types=None):
        """
                ...
                for flow_type in flow_types:
                    ...
                    if flow_type == "total":
                        self[edge[0]][edge[1]]["flow_" + flow_type] = sum(
                            [
                                shipment["quantity"] for shipment in self[edge[0]][edge[1]]["shipments"].values()
                            ]
                        )
                    # or flow category
                    elif flow_type in [
    ...
    def evaluate_normal_traffic(self, sectorId_to_volumeCoef=None):
        ...
        for edge in self.edges():
            if self[edge[0]][edge[1]]["type"] == "virtual":
                continue
            self[edge[0]][edge[1]]["traffic_normal"] = self[edge[0]][edge[1]][
                "traffic_current"
            ]
    ...
    def evaluate_traffic(self, sectorId_to_volumeCoef=None):
        # If we have a correspondence of sector monetary flow to volume,
        # we identify the sector that generate volume

        # If we have a correspondence of sector monetary flow to volume,
        # we use volume
        if sectorId_to_volumeCoef is not None:
            self.evaluate_trafic_w_correspondence(sectorId_to_volumeCoef)
        # Otherwise we use monetary flow directly
        else:
            self.evaluate_traffic_no_corr()

    def evaluate_traffic_no_corr(self):
        for edge in self.edges():
            if self[edge[0]][edge[1]]["type"] == "virtual":
                continue
    ...
    def evaluate_trafic_w_correspondence(self, sectorId_to_volumeCoef):
        ...
        for edge in self.edges():
            ...
            if sectorId_to_volumeCoef is not None:
                ...
                for sector_id in sectors_causing_congestion:
                    ...
                    volume += sectorId_to_volumeCoef[sector_id] * sum(
                        list_montetary_flows
                    )
