...
def exportFirmODPointTable(
        firm_list,
        firm_table,
        household_table,
        filepath_road_nodes,
        export_firm_table,
        export_odpoint_table,
        export_folder,
):
    # Get info per firm
    imports = pd.Series(
        {
            firm.pid: sum(
                [val for key, val in firm.purchase_plan.items() if str(key)[0] == "C"]
            )
    ...
    production_table = pd.Series(
        {firm.pid: firm.production_target for firm in firm_list},
    ...
    b2c_share = pd.Series(
        {
            firm.pid: sum(
                [
                    val["share"]
                    for key, val in firm.clients.items()
                    if str(key)[0] == "h"
                ]
            )
            for firm in firm_list
    ...
    production_table["production_toC"] = (
            production_table["total_production"] * production_table["b2c_share"]
    )
    production_table["production_toB"] = (
            production_table["total_production"] - production_table["production_toC"]
    )
    production_table["production_exported"] = (
            production_table["total_production"] * production_table["export_share"]
    )
    ...
    odpoint_table_household = household_table.groupby("odpoint", as_index=False)[
        "population"
    ].sum()
    ...
    prod_per_sector_ODpoint_table = (
        firm_table.groupby(["odpoint", "sector"])["total_production"]
        .sum()
        .unstack()
        .fillna(0)
        .reset_index()
    ...
    if export_firm_table:
        ...
        household_table.to_csv(
            os.path.join(export_folder, "household_table.csv"), index=False
        )

    if export_odpoint_table:
        odpoint_table.to_csv(
            os.path.join(export_folder, "odpoint_production_table.csv"), index=False
        )
...
def exportODPointLayer(odpoint_table, filepath_road_nodes, export_folder):
    ...
    res.to_file(
        os.path.join(export_folder, "odpoints_output.geojson"), driver="GeoJSON"
    )


def exportDistrictSectorTable(filtered_district_sector_table, export_folder):
    filtered_district_sector_table.to_csv(
        os.path.join(export_folder, "filtered_district_sector_table.csv"), index=False
    )
    ...
    country_table = pd.DataFrame(
        {
            ...
            "purchases": [
                sum(country.purchase_plan.values()) for country in country_list
            ],
            "purchases_from_countries": [
                sum(
                    [
                        value if isinstance(key, str) else 0
                        for key, value in country.purchase_plan.items()
                    ]
                )
                for country in country_list
            ],
            "purchases_from_firms": [
                sum(
                    [
                        value if isinstance(key, int) else 0
                        for key, value in country.purchase_plan.items()
                    ]
                )
                for country in country_list
...
def exportEdgelistTable(supply_chain_network, export_folder):
    ...
    edgelist_table.to_csv(
        os.path.join(export_folder, "edgelist_table.csv"), index=False
    )

    # Log some key indicators, taking into account all firms
    av_km_btw_supplier_buyer = edgelist_table["distance"].mean()
    logging.info(
        "Average distance between supplier and buyer: "
        + "{:.01f}".format(av_km_btw_supplier_buyer)
        + " km"
    )
    av_km_btw_supplier_buyer_weighted_by_flow = (
                                                        edgelist_table["distance"] * edgelist_table["flow"]
                                                ).sum() / edgelist_table["flow"].sum()
    logging.info(
        "Average distance between supplier and buyer, weighted by the traded quantity: "
        + "{:.01f}".format(av_km_btw_supplier_buyer_weighted_by_flow)
        + " km"
    )

    # Log some key indicators, excluding service firms
    boolindex = (edgelist_table["supplier_odpoint"] != -1) & (
            edgelist_table["buyer_odpoint"] != -1
    )
    av_km_btw_supplier_buyer_no_service = edgelist_table.loc[
        boolindex, "distance"
    ].mean()
    logging.info(
        "Average distance between supplier and buyer, excluding service firms: "
        + "{:.01f}".format(av_km_btw_supplier_buyer_no_service)
        + " km"
    )
    av_km_btw_supplier_buyer_weighted_by_flow_no_service = (
                                                                   edgelist_table.loc[boolindex, "distance"]
                                                                   * edgelist_table.loc[boolindex, "flow"]
                                                           ).sum() / edgelist_table.loc[boolindex, "flow"].sum()
    logging.info(
        "Average distance between supplier and buyer, excluding service firms"
        + ", weighted by the traded quantity: "
        + "{:.01f}".format(av_km_btw_supplier_buyer_weighted_by_flow_no_service)
        + " km"
    )
...
def analyzeSupplyChainFlows(sc_network, firm_list, export_folder):
    ...
    io_flows = [
        [
            sc_network[edge[0]][edge[1]]["object"].delivery,
            sc_network[edge[0]][edge[1]]["object"].supplier_id,
            sc_network[edge[0]][edge[1]]["object"].buyer_id,
        ]
        for edge in sc_network.edges
    ]
    ...
    domestic_flows = io_flows[
        (io_flows["supplier_id"].apply(lambda x: isinstance(x, int)))
        & (io_flows["buyer_id"].apply(lambda x: isinstance(x, int)))
        ]
    ...
    domestic_b2c_flows_per_sector = (
        domestic_b2c_flows.groupby("from_sector")["quantity"].sum().reset_index()
    )
    ...
    domestic_b2b_flows_per_sector = (
        domestic_b2b_flows.groupby(["from_sector", "to_sector"])["quantity"]
        .sum()
        .reset_index()
    )
    ...
    for i in range(domestic_b2b_flows_per_sector.shape[0]):
        observed_io_matrix.loc[
            domestic_b2b_flows_per_sector["from_sector"].iloc[i],
            domestic_b2b_flows_per_sector["to_sector"].iloc[i],
        ] = domestic_b2b_flows_per_sector["quantity"].iloc[i]

    # Analyze import B2B flows
    import_flows = io_flows[
        (io_flows["supplier_id"].apply(lambda x: isinstance(x, str)))
        & (io_flows["buyer_id"].apply(lambda x: isinstance(x, int)))
        ]
    import_b2b_flows = import_flows[import_flows["buyer_id"] >= 0].copy()
    import_b2b_flows_per_country = (
        import_b2b_flows.groupby("supplier_id")["quantity"].sum().reset_index()
    )
    import_b2b_flows["to_sector"] = import_b2b_flows["buyer_id"].map(
        dic_firmid_to_sectorid
    )
    import_b2b_flows_per_sector = (
        import_b2b_flows.groupby("to_sector")["quantity"].sum().reset_index()
    )
    ...
    export_flows = io_flows[
        (io_flows["supplier_id"].apply(lambda x: isinstance(x, int)))
        & (io_flows["buyer_id"].apply(lambda x: isinstance(x, str)))
        ].copy()
    export_flows_per_country = (
        export_flows.groupby("buyer_id")["quantity"].sum().reset_index()
    )
    export_flows["from_sector"] = export_flows["supplier_id"].map(
        dic_firmid_to_sectorid
    )
    export_flows_per_sector = (
        export_flows.groupby("from_sector")["quantity"].sum().reset_index()
    )

    # Analyze transit flows
    transit_flows = io_flows[
        (io_flows["supplier_id"].apply(lambda x: isinstance(x, str)))
        & (io_flows["buyer_id"].apply(lambda x: isinstance(x, str)))
        ].copy()
    transit_countries = (
        pd.Series(
            list(set(transit_flows["supplier_id"]) | set(transit_flows["buyer_id"]))
        )
        .sort_values()
        .tolist()
    )
    ...
    for i in range(transit_flows.shape[0]):
        country_to_country_transit_matrix.loc[
            transit_flows["supplier_id"].iloc[i], transit_flows["buyer_id"].iloc[i]
        ] = transit_flows["quantity"].iloc[i]

    # Form final consumption
    final_consumption = domestic_b2c_flows_per_sector.append(
        pd.DataFrame(
            index=["import"],
            data={
                "from_sector": "import",
                "quantity": import_b2c_flows["quantity"].sum(),
            },
        )
    )
    ...
    observed_io_matrix = pd.concat(
        [
            pd.concat(
                [
                    observed_io_matrix,
                    import_b2b_flows_per_sector.set_index("to_sector")
            ...
            export_flows_per_sector.set_index("from_sector").rename(
    ...
    # Export Report
    final_consumption.to_csv(
        os.path.join(export_folder, "initial_final_consumption.csv"), index=False
    )
    observed_io_matrix.to_csv(
        os.path.join(export_folder, "initial_sector_io_matrix.csv"), index=True
    )
    country_to_country_transit_matrix.to_csv(
        os.path.join(export_folder, "initial_transit_matrix.csv"), index=True
    )
    import_b2b_flows_per_country.to_csv(
        os.path.join(export_folder, "initial_import_b2b_flows_per_country.csv"),
        index=False,
    )
    export_flows_per_country.to_csv(
        os.path.join(export_folder, "initial_export_flows_per_country.csv"), index=False
    )
...
def exportFirmTS(what, observer, export_folder):
    ts = pd.DataFrame(
        {
            t: {firm_id: val[what] for firm_id, val in observer.firms[t].items()}
    ...
    firm_avinventoryduration_ts = pd.DataFrame(
        {
            t: {
                firm_id: sum(val["inventory_duration"].values())
                         / len(val["inventory_duration"].values())
                for firm_id, val in observer.firms[t].items()
            }
            for t in observer.firms.keys()
        }
    ).transpose()
    firm_avinventoryduration_ts.to_csv(
        os.path.join(export_folder, "firm_avinventoryduration_ts.csv"), sep=","
    )
...
def initializeResPerFirmExportFile(export_folder, firm_list):
    extra_spending_export_file = open(
        os.path.join(export_folder, "hh_extra_spending_per_firm.csv"), "w"
    )
    missing_consumption_export_file = open(
        os.path.join(export_folder, "hh_missing_consumption_per_firm.csv"), "w"
    )
...
def writeCriticalityResults(
        criticality_export_file, observer, disruption, disruption_duration, computation_time
):
    criticality_export_file.write(
        str(disruption["node"])
        + ", "
...
def writeResPerFirmResults(
        extra_spending_export_file, missing_consumption_export_file, observer, disruption
):
...
def exportParameters(exp_folder):
    copy_command = {"nt": "copy", "posix": "cp"}
    os.system(
        copy_command[os.name]
        + " "
