        ...
        try:
            return min(
                [
                    ...
                    for input_id, val in input_mix.items()
                ]
            )
        except KeyError:
            return 0
...
def purchase_planning_function(
    estimated_need, inventory, inventory_duration_target=1, reactivity_rate=1
):
...
def set_initial_conditions(
    graph, firm_list, household_list, country_list, mode="equilibrium"
):
    ...
    if mode == "equilibrium":
        initialize_at_equilibrium(graph, firm_list, household_list, country_list)
...
def build_final_demand_vector(household_list, country_list, firm_list):
    ...
    for country in country_list:
        for supplier_id, quantity in country.purchase_plan.items():
            if isinstance(
                supplier_id, int
            ):  # we only consider purchase from firms, not from other countries
...
def initialize_at_equilibrium(
    graph: nx.DiGraph, firm_list: list, household_list: list, country_list: list
):
    ...
    firm_connectivity_matrix = nx.adjacency_matrix(
        graph.subgraph(list(graph.nodes)[:-1]), weight="weight", nodelist=firm_list
    ).todense()
    ...
    final_demand_vector = build_final_demand_vector(
        household_list, country_list, firm_list
    )

    # Solve the input--output equation
    eq_production_vector = np.linalg.solve(
        np.eye(n) - firm_connectivity_matrix, final_demand_vector
    )
    ...
    domestic_input_cost_vector = np.multiply(
        firm_connectivity_matrix.sum(axis=0).reshape((n, 1)), eq_production_vector
    )
    import_input_cost_vector = np.multiply(
        np.array(import_weight_per_firm).reshape((n, 1)), eq_production_vector
    )
    ...
    transport_cost_vector = np.multiply(
        eq_production_vector, proportion_of_transport_cost_vector
    )
    # Compute other costs based on margin
    margin = np.array([firm.target_margin for firm in firm_list]).reshape((n, 1))
    other_cost_vector = (
        np.multiply(eq_production_vector, (1 - margin))
        - input_cost_vector
        - transport_cost_vector
    )
...
def determine_suppliers_and_weights(
    potential_supplier_pid, nb_selected_suppliers, firm_list, mode
):
    ...
    if "importance_export" in mode.keys():
        importance_of_each = rescale_values(
            [
                ...
                if firm_list[firm_pid].odpoint
                in mode["importance_export"]["export_odpoints"]
        ...
        importance_of_each = rescale_values(
            [firm_list[firm_pid].importance for firm_pid in potential_supplier_pid]
        )

    else:
        raise ValueError("mode variable does not contain importance")
    # Select supplier
    ...
    supplier_weights = generate_weights_from_list(
        [firm_list[firm_pid].importance for firm_pid in selected_supplier_ids]
    )
...
def identify_firms_in_each_sector(firm_list):
    ...
    dic_sector_to_firmid = (
        firm_id_each_sector.groupby("sector")["firm"].apply(lambda x: list(x)).to_dict()
    )
...
def allAgentsReceiveProducts(
    G, firm_list, household_list, country_list, T, sectors_no_transport_network
):
    for household in household_list:
...
def rescale_values(
    input_list, minimum=0.1, maximum=1, max_val=None, alpha=1, normalize=False
):
    ...
    if max_val == min_val:
        ...
        res = [
            minimum
            + (((val - min_val) / (max_val - min_val)) ** alpha) * (maximum - minimum)
...
def evaluate_sectoral_shock(firm_table, disrupted_node):
    disrupted_sectoral_production = (
        firm_table[firm_table["odpoint"].isin(disrupted_node)]
        .groupby("sector_id")["total_production"]
        .sum()
    )
    normal_sectoral_production = firm_table.groupby("sector_id")[
        "total_production"
    ].sum()
    consolidated_table = pd.concat(
        [disrupted_sectoral_production, normal_sectoral_production], axis=1
    ).fillna(0)
...
def apply_sectoral_shocks(sectoral_shock, firm_list):
    for firm in firm_list:
        if firm.sector in sectoral_shock.index:
            firm.production_capacity = firm.production_target * (
                1 - sectoral_shock[firm.sector]
            )
...
def calculate_distance_between_agents(agentA, agentB):
    if (agentA.odpoint == -1) or (agentB.odpoint == -1):
        logging.warning(
            "Try to calculate distance between agents, but one of them does not have real odpoint"
        )
        return 1
    else:
...
def select_supplier_from_list(
    ...
    if importance:
        importance_of_each = rescale_values(
            [firm_list[firm_id].importance for firm_id in potential_firm_ids]
        )
        importance_weight = np.array(importance_of_each)
    else:
        importance_weight = None
        importance_of_each = None
...
def agent_receive_products_and_pay(
    agent, graph, transport_network, sectors_no_transport_network
):
    # reset variable
    ...
    for edge in graph.in_edges(agent):
        if graph[edge[0]][agent]["object"].product_type in sectors_no_transport_network:
            ...
            agent_receive_shipment_and_pay(
                agent, graph[edge[0]][agent]["object"], transport_network
            )
...
def agent_receive_service_and_pay(agent, commercial_link):
    ...
    # Update indicator
    agent_update_indicator(
        agent, quantity_delivered, commercial_link.price, commercial_link
    )
...
def agent_update_indicator(agent, quantity_delivered, price, commercial_link):
    ...
    if agent.agent_type == "country":
        ...
        agent.spending_per_retailer[commercial_link.supplier_id] = (
            quantity_delivered * price
        )
        agent.tot_spending += quantity_delivered * price
        agent.extra_spending += quantity_delivered * (price - commercial_link.eq_price)
        agent.consumption_loss = (
            agent.purchase_plan[commercial_link.supplier_id] - quantity_delivered
    ...
    if abs(commercial_link.delivery - quantity_delivered) > 1e-6:
        logging.debug(
            "Agent "
            + str(agent.pid)
            + ": quantity delivered by "
            + str(commercial_link.supplier_id)
            + " is "
            + str(quantity_delivered)
            + ". It was supposed to be "
            + str(commercial_link.delivery)
            + "."
        )
